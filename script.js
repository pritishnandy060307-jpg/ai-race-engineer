...
function calculateGear() {
    const engineRpm = value("engineRpmInput");
    const gearRatio = value("gearRatioInput");
    const finalDriveRatio = value("finalDriveInput");
    const tyreDiameterM = value("tyreDiameterInput");
    ...
    if (![engineRpm, gearRatio, finalDriveRatio, tyreDiameterM].every(Number.isFinite) ||
        ...
        show("calculatorError", "Please enter valid positive values.");
        return;
    }
    ...
    show("calculatorError", "");
}

function calculateInflation() {
    const result = calculateInflationLayer({
        velocity: value("inflVelocity"),
        lengthScale: value("inflLength"),
        viscosity: value("inflViscosity"),
        density: value("inflDensity"),
        targetYPlus: value("inflYPlus"),
        layers: value("inflLayers")
    });

    if (!result) {
        show("inflError", "Enter valid values. Layers must be at least 2 and Reynolds number must support the correlation.");
        return;
    }
...
function calculateTurbulence() {
    const result = calculateInletTurbulence({
        velocity: value("turbVelocity"),
        intensityPercent: value("turbIntensity"),
        lengthScale: value("turbLength")
    });

    if (!result) {
        show("turbError", "Enter valid velocity, intensity and length scale.");
        return;
    }
...
function calculateParticle() {
    ...
    const properties = fluid === "air"
        ? { density: 1.225, viscosity: 1.81e-5 }
        : { density: 998, viscosity: 1.002e-3 };

    const result = calculateParticleSettling({
        diameter: value("particleDiameter"),
        particleDensity: value("particleDensity"),
        fluidDensity: properties.density,
        fluidViscosity: properties.viscosity
    });

    if (!result) {
        show("particleError", "Particle density must exceed fluid density and all values must be valid.");
        return;
    }
...
function calculateHumidityTool() {
    const result = calculateHumidity({
        temperatureC: value("humidityTemp"),
        pressurePa: value("humidityPressure"),
        relativeHumidity: value("humidityRH")
    });

    if (!result) {
        show("humidityError", "Enter valid pressure and relative humidity (0–1).");
        return;
    }
...
function completeLap() {
    ...
    if (raceState.lap.bestTimeSeconds === null || completedLapTime < raceState.lap.bestTimeSeconds) {
