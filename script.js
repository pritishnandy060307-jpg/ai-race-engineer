...
function calculateInflation() {
    ...
    if (!result) { show("inflError", "Enter valid values. Layers must be at least 2 and Reynolds number must support the correlation."); return; }
    show("inflError", ""); show("inflFirst", result.firstLayer.toExponential(4)); show("inflFinal", result.finalLayer.toExponential(4)); show("inflDelta", result.delta99.toExponential(4)); show("inflGrowth", fmt(result.growthRatio, 4));
}
...
function calculateTurbulence() {
    ...
    if (!result) { show("turbError", "Enter valid velocity, intensity and length scale."); return; }
    show("turbError", ""); show("turbK", result.k.toExponential(4)); show("turbOmega", result.omega.toExponential(4)); show("turbEpsilon", result.epsilon.toExponential(4)); show("turbNu", result.turbulentViscosity.toExponential(4));
}
...
function calculateParticle() {
    ...
    if (!result) { show("particleError", "Particle density must exceed fluid density and all values must be valid."); return; }
    show("particleError", ""); show("particleRe", fmt(result.reynolds, 4)); show("particleCd", fmt(result.dragCoefficient, 4)); show("particleVelocity", result.velocity.toExponential(4));
}
...
function calculateHumidityTool() {
    ...
    if (!result) { show("humidityError", "Enter valid pressure and relative humidity (0–1)."); return; }
    show("humidityError", ""); show("humiditySpecific", result.specificHumidity.toExponential(4)); show("humidityAbsolute", result.absoluteHumidity.toExponential(4)); show("humidityMassFraction", result.absoluteHumidity.toExponential(4));
}
