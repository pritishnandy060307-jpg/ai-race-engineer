...
const calculateWeightTransferButton = document.querySelector("#calculateWeightTransferButton");
...
calculateWeightTransferButton?.addEventListener("click", calculateWeightTransferTool);
...
function calculateWeightTransferTool() { const result = calculateWeightTransfer({ mass: value("wtMass"), cgHeight: value("wtCgHeight"), wheelbase: value("wtWheelbase"), trackWidth: value("wtTrackWidth"), longitudinalAccelG: value("wtLongG"), lateralAccelG: value("wtLatG"), staticFrontPercent: value("wtFrontPercent") }); if (!result) { show("wtError", "Enter valid positive vehicle dimensions and a front weight percentage between 0 and 100."); return; } show("wtError", ""); show("wtLongTransfer", `${result.longitudinalTransfer.toFixed(1)} N`); show("wtLatTransfer", `${result.lateralTransfer.toFixed(1)} N`); show("wtFrontLoad", `${result.frontLoad.toFixed(1)} N`); show("wtRearLoad", `${result.rearLoad.toFixed(1)} N`); show("wtInsideLoad", `${result.insideLoad.toFixed(1)} N`); show("wtOutsideLoad", `${result.outsideLoad.toFixed(1)} N`); }

function setupNavigation() {
    if (document.querySelector("#toolNavigation")) return;
...
function categoryForSection(section) {
    ...
    if (label.includes("gear ratio")) return "powertrain";
    if (label.includes("brake bias") || label.includes("tyre temperature")) return "brakes";
    if (label.includes("stopping distance") || label.includes("cornering speed") || label.includes("lateral g") || label.includes("weight transfer") || label.includes("power-to-weight") || label.includes("acceleration") || label.includes("lap time") || label.includes("lap delta")) return "vehicle";
    if (label.includes("downforce") || label.includes("inflation layer") || label.includes("inlet turbulence")) return "aero";
    if (label.includes("particle settling") || label.includes("humidity")) return "fluid";
    return "dashboard";
}
